...
Template.viewMedia.helpers({
  viewMediaById() {
    const cat = [],
      ...
      lang = [];
    const id = FlowRouter.getParam('id');
    const mediaArr = TempMedia.find({ media_id: id }).fetch();
    media.push(mediaArr[0]);
    media[0].MasterCat.forEach(e => {
      cat.push(e.Category_name);
    });
    media[0].MasterTag.forEach(e => {
      tag.push(e.hash_tag);
    });
    media[0].MasterLang.forEach(e => {
