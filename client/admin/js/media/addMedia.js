...
Template.addMedia.onCreated(function() {
  let self = this;
...
Template.addMedia.events({
  ...
  'submit .addAVForm': function(e) {
    e.preventDefault();
    const Type = e.target.querySelector('#mediaType').value,
      Source = e.target.querySelector('#sourceURL').value,
      For = e.target.querySelector('#mediaUserType').value,
      Title = e.target.querySelector('#mediaContentTitle').value,
      editorHtml = e.target.querySelector('.ql-editor').innerHTML,
      editorText = e.target.querySelector('.ql-editor').innerText,
      Genre = multiSelectArray('media-genre'),
      Language = multiSelectArray('media-language'),
      Tags = multiSelectArray('media-tags'),
      city = $('#mediacity').val(),
      state = $('#mediastate').val(),
      country = $('#mediacountry').val(),
      lat = $('#medialat').val(),
      lng = $('#medialng').val(),
      formatted_address = $('#mediaformatted_address').val(),
      place_id = $('#mediaplace_id').val(),
      removeStylesArr = [
        'checkAddedMediaTitleInput',
        'checkAddedMediaTitleInvalidMessage',
        'checkAddedMediaTitleLengthMessage',
        'checkAddedMediaTitleNumberMessage',
        'checkAddedMediaDescInput',
        'checkAddedMediaDescMessage',
        'checkAddedMediaDescNumberMessage'
      ];
    ...
    if (!validUrl.isUri(Source)) {
      ...
      addErrOutline('checkAddedMediaTitleInput');
      addErrClass('checkAddedMediaTitleLengthMessage');
      setTimeout(() => {
        removeStyles('checkAddedMediaTitleInput');
        removeStyles('checkAddedMediaTitleLengthMessage');
      }, 2000);
    } else if (Title.replace(/[^\w]/g, '') === '') {
      addErrOutline('checkAddedMediaTitleInput');
      addErrClass('checkAddedMediaTitleInvalidMessage');
      setTimeout(() => {
        removeStyles('checkAddedMediaTitleInput');
        removeStyles('checkAddedMediaTitleInvalidMessage');
      }, 2000);
    } else if (!isNaN(Title)) {
      addErrOutline('checkAddedMediaTitleInput');
      addErrClass('checkAddedMediaTitleNumberMessage');
      setTimeout(() => {
        removeStyles('checkAddedMediaTitleInput');
        removeStyles('checkAddedMediaTitleNumberMessage');
      }, 2000);
